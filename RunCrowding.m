function [display,config,result] = RunCrowding(subjectName,subjectIni,ver,eccen)
...
config.startPoint = config.eccen * .5 * 1.3; % 1.3 times critical spacing Bouma
...
while trial <= config.nTotalTrials
    if config.randOrder(trial) == 1
        config.spacing(trial) = max(1, 10^QuestMean(result.q(1))); % Hard-coded
    elseif config.randOrder(trial) == 2
        config.spacing(trial) = max(1, 10^QuestMean(result.q(2))); % Hard-coded
    end
    
    if config.spacing(trial) > config.eccen
        config.spacing(trial) = config.eccen-1;
    end
    config.spacing
    ...
    if ~config.loc(trial) % left
        config.rect(1,:) = [display.cx - config.eccen*display.ppd - config.spacing(trial)*display.ppd, ...
            display.cy, display.cx - config.eccen*display.ppd - config.spacing(trial)*display.ppd, ...
            display.cy];
        config.rect(2,:) = [display.cx - config.eccen*display.ppd, ...
            display.cy - config.spacing(trial)*display.ppd, display.cx - config.eccen*display.ppd, ...
            display.cy - config.spacing(trial)*display.ppd];
        config.rect(3,:) = [display.cx - config.eccen*display.ppd + config.spacing(trial)*display.ppd, ...
            display.cy, display.cx - config.eccen*display.ppd + config.spacing(trial)*display.ppd, ...
            display.cy];
        config.rect(4,:) = [display.cx - config.eccen*display.ppd, ...
            display.cy + config.spacing(trial)*display.ppd, display.cx - config.eccen*display.ppd, ...
            display.cy + config.spacing(trial)*display.ppd];
        config.rect(5,:) = [display.cx - config.eccen*display.ppd, ...
            display.cy, display.cx - config.eccen*display.ppd, ...
            display.cy];
    else
        config.rect(1,:) = [display.cx + config.eccen*display.ppd - config.spacing(trial)*display.ppd, ...
            display.cy, display.cx + config.eccen*display.ppd - config.spacing(trial)*display.ppd, ...
...
%         score = score + 1;
        ...
        if stair1 == 1 || stair2 == 1 || stair1 == 2 || stair2 == 2
            repeatAgain = 1;
        end
    end
    Screen('Flip',display.wPtr);
...
plot(result.quest(:,3,1), 'ro-');
plot(result.quest(:,3,2), 'bo-');
